...
快照重建阶段新增 `distance_weighted_kamada_kawai` 布局：

- 基于 NetworkX 图计算节点坐标
- 边权、相关文章数和关系数越高，布局距离越短
- 非直连节点通过图最短路径间接拉开距离
- 多个连通分量分别布局后再打包，避免孤立分量重叠
- 节点 payload 新增 `layout_x` / `layout_y`，快照视图同步返回 `layout_mode`
...
- 1 跳邻域 / 2 跳邻域
- 路径高亮
- 优先使用后端 `layout_x` / `layout_y` 进行距离感知渲染
- 缩放、平移、重置视图
