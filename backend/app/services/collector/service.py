...
class CollectionService:
    ...
    def collect_all(
        ...
        try:
            ...
            logger.info("\n[IndustryGraph] Importing collected articles into industry graph documents...")
            with db.get_session() as session:
                industry_graph_service = IndustryGraphService(db=session, ai_analyzer=self.ai_analyzer)
                import_result = industry_graph_service.import_articles()
                stats["industry_graph_imported"] = import_result.get("imported", 0)
                stats["industry_graph_skipped"] = import_result.get("skipped", 0)
                logger.info(
                    "[IndustryGraph] Import completed: imported=%s skipped=%s",
                    stats["industry_graph_imported"],
                    stats["industry_graph_skipped"],
                )
        except Exception as e:
            logger.warning("[IndustryGraph] Import failed but collection will continue: %s", e)
            stats["industry_graph_imported"] = 0
            stats["industry_graph_skipped"] = 0
